...
const THEMES: Record<"light" | "dark", Theme> = {
  light: {
    ...
    categorical: {
      ...
      "cat-2": "#12a374",
      "cat-3":
        "#bf8200" /* darkened from --color-warning's #d99400 — see globals.css */,
      "cat-4": "#7c5cd6",
      "cat-5": "#2b87c4",
      "cat-6": "#c0823a",
    },
    checks: [
      ...
      [
        ...
        "#ffffff",
        4.5,
        "11px section headings — was 3.20:1",
      ],
      [
        "faint on sunken",
        "#616d76",
        "#f1f5f8",
        4.5,
        "tertiary text on a tinted chip",
      ],
      [
        "accent on surface",
        "#2b3990",
      ...
      [
        ...
        "#f7f9fb",
        4.5,
        "accent links on canvas",
      ],
      ["accent-text on accent-soft", "#2b3990", "#f0f2f9", 4.5, "callout text"],
      [
        "control-line on surface",
        "#6b7c89",
        "#ffffff",
        3.0,
        "pickers, choice controls, hover accents",
      ],
      [
        "field-line on surface",
        "#8492a0",
        "#ffffff",
        3.0,
        "resting border, text and select inputs",
      ],
      ["success-text on surface", "#417739", "#ffffff", 4.5, "success copy"],
      [
        "success-text on success-soft",
        "#417739",
        "#f1f8ef",
        4.5,
        "success callout",
      ...
      [
        "white on fill-strong",
        "#ffffff",
        "#0a1f2d",
        4.5,
        "fixed fill, both themes — ink fill label, headers, dialogs",
      ],
      [
        "white on accent",
        "#ffffff",
        "#2b3990",
        4.5,
        "fixed fill, both themes — primary/accent/approve button label",
      ],
      [
        "fill-strong on success",
        "#0a1f2d",
        "#8ac97d",
        4.5,
        "fixed fill, both themes — green secondary on hover",
      ],
      [
        ...
        "#b3261e",
        4.5,
        "fixed fill, both themes — destructive button label",
      ],
      [
        "white on success-strong",
        "#ffffff",
        "#529546",
        3.0,
...
for (const [themeName, theme] of Object.entries(THEMES)) {
  ...
  for (const [token, fg] of Object.entries(theme.textTokens)) {
    for (const [bgName, bg] of Object.entries(theme.backgrounds)) {
      ...
      if (ratio < 4.5) {
        rows.push(
          `  FAIL  ${ratio.toFixed(2).padStart(6)}:1  needs 4.5  ${label}`,
        );
  ...
  for (const [name, fg] of Object.entries(theme.categorical)) {
    for (const bgName of ["surface", "canvas"]) {
      ...
      if (ratio < 3.0) {
        rows.push(
          `  FAIL  ${ratio.toFixed(2).padStart(6)}:1  needs 3.0  ${label}`,
        );
  ...
  rows.push(
    `  pass  [${themeName}] ${Object.keys(theme.categorical).length * 2} categorical pairs checked`,
  );
  ...
  for (const [name, fg, bg, min, why] of theme.checks) {
    ...
    if (!pass)
      failures.push(`${label} is ${ratio.toFixed(2)}:1, needs ${min}:1`);
  }
}
