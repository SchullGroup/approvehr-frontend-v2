...
| **You'll see** | A dialog with **What to call it**, the dates, **Answers due by**, and five collapsed sections |
...
#### 8. *Optional* — decide whether the Owner and the HR manager are appraised

Two tick boxes on the same settings card, and **both are off**. So on a new period the person who
owns the company and whoever runs HR are handed no form of their own unless you say otherwise.

| | |
|---|---|
| **Where** | `/performance/periods/{id}`, or the **Appraise the Owner and the HR manager** section of the Start dialog |
| **Who** | HR / Owner |
| **Click** | **Appraise the Owner in this period** · **Appraise the HR manager in this period** |
| **You'll see** | Nothing on this screen. What changes is who the period hands a form to when you start it — one more person for each box you tick |

Two boxes rather than one, because they are two unrelated questions. An HR manager reports to
somebody like anybody else, so a company that does not appraise its owner may still want its HR
manager appraised.

> 💡 **Left out is not let off.** Whoever is unticked still writes their own team's reviews and still
> signs them off. All they lose is a form *about themselves* — which is also the point, because it is
> what stops them finishing the period counted as unscored.

> ⚠️ **Decide it before you start the period.** Forms are handed out at the moment you start. The
> boxes still work afterwards and the product will not stop you, but ticking one then changes nothing
> for anybody who already has a form.
...
#### 11. *Optional* — give the same objective to several people at once
...
#### 16. *Optional* — log what you did each week
...
#### 21. *Optional* — ask colleagues for feedback
...
#### 24. *Optional* — place people on the nine-box
...
#### 27. *Optional* — send one review back to be redone
...
| **The Owner and the HR manager get no form.** | Both off on every new period, and each is its own tick box. The Owner reports to nobody, so the reporting line has no one to put on their form; the HR manager would otherwise be marking themselves against questions they wrote. Neither is let off anything — both still appraise their own teams. Tick either box on the period to include them. |
...
*Written against the product as it stands on 23 September 2026. Every button label, field name and
